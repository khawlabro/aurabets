...
class BetSmartApp {
    constructor() {
        ...
        this.bets = []; // Empty array (we'll load from JSON)
        this.gameData = {};
        this.selectedSport = 'All Sports';
        this.sortBy = 'value';
        this.highValueOnly = false;
        this.ADMIN_PIN = "1009";
        this.DATA_URL = 'data/bets.json'; // Path to JSON file
        this.init();
    }
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error("Couldn't load bets.json");
        }
        
        const data = await response.json();
        
        // 2. Use loaded data if available
        ...
        // 3. Fallback to hardcoded data if JSON fails
        this.bets = [
            // Paste ALL your current bet data here
            // (The entire array you currently have in loadData())
            {
                id: 1,
                sport: "UFC",
                event: "Jon Jones vs Stipe Miocic",
                // ... keep ALL existing bet data ...
            },
            // ... all other bets ...
        ];
